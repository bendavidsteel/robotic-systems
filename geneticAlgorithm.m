...
for i = 1:POPULATION
    ...
    chromosomes(i, :) = [MAX_POINTS, GEN_RADIUS, GEN_TIME_OUT, MIN_WALL_DIST, MAX_GRAPH_DIST, NO_GRAPH_ITERATIONS, TIME_OUT, RAD_EXPANSION, TUNE_FACTOR];
    %running and evaluating algorithm with a set of arguments
    scores(i) = pathFindingTesting(MAX_POINTS, GEN_RADIUS, GEN_TIME_OUT, MIN_WALL_DIST, MAX_GRAPH_DIST, NO_GRAPH_ITERATIONS, TIME_OUT, RAD_EXPANSION, TUNE_FACTOR);
end
...
while true
    ...
    for i = 1:POPULATION
        ...
        if (rand(1) < MUTATION_RATE)
            ...
            MAX_POINTS = parents(gene_choices(1),1);
        end
        
        if (rand(1) < MUTATION_RATE)
            GEN_RADIUS = normrnd(GEN_RADIUS, 1);
        else
            GEN_RADIUS = parents(gene_choices(2),2);
        end
        
        if (rand(1) < MUTATION_RATE)
            GEN_TIME_OUT = round(normrnd(GEN_TIME_OUT, 20));
        else
            GEN_TIME_OUT = parents(gene_choices(3),3);
        end
        
        if (rand(1) < MUTATION_RATE)
            MIN_WALL_DIST = 1;
        else
            MIN_WALL_DIST = parents(gene_choices(4),4);
        end
        
        if (rand(1) < MUTATION_RATE)
            MAX_GRAPH_DIST = round(normrnd(MAX_GRAPH_DIST, 10));
        else
            MAX_GRAPH_DIST = parents(gene_choices(5),5);
        end
        
        if (rand(1) < MUTATION_RATE)
            NO_GRAPH_ITERATIONS = round(normrnd(NO_GRAPH_ITERATIONS, 10));
        else
            NO_GRAPH_ITERATIONS = parents(gene_choices(6),6);
        end
        
        if (rand(1) < MUTATION_RATE)
            TIME_OUT = round(normrnd(TIME_OUT, 10));
        else
            TIME_OUT = parents(gene_choices(7),7);
        end
        
        if (rand(1) < MUTATION_RATE)
            RAD_EXPANSION = normrnd(RAD_EXPANSION, 0.1);
        else
            RAD_EXPANSION = parents(gene_choices(8),8);
        end
        
        if (rand(1) < MUTATION_RATE)
            TUNE_FACTOR = normrnd(TUNE_FACTOR, 0.1);
        else
            TUNE_FACTOR = parents(gene_choices(9),9);
        end
